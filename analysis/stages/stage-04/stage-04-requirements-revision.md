...
| D-018 | Legacy web name search is an unsupported placeholder and failed lookups can leave stale customer state selected. | Implement normalized name search in the target. A failed lookup must clear stale customer state and disable mutations while showing a recoverable error. |
| D-019 | Legacy account portfolio channels expose fixed six-, ten-, or twenty-entry limits and inconsistent presentation. | Return the complete authorized portfolio through bounded target pagination. Do not preserve channel-specific display caps. |
| D-020 | Legacy screens sometimes request a sort code while other flows obtain the single bank sort code internally. | Store bank/account sort code as configured system-managed data. Derive it from the selected account or bank configuration; do not trust operator-entered sort code for target commands. |
| D-021 | Legacy data generators destructively clear tables, expose range/seed controls, and loaders can stop after partially committed batches. | Replace them with explicit deterministic reset/demo commands and staged import. Validate first, promote atomically, retain a run ledger, and support resumable retry without treating partial staging as trusted data. |
| D-022 | Unused bank-identity helpers, fixed-account diagnostics, a broken TOC generator, and other operational utilities do not provide banking capabilities. | Move bank identity to configuration and do not port unused/broken diagnostic or documentation utilities. Preserve their disposition in traceability only. |
| D-023 | Legacy customer/account counter updates rely on runtime transaction boundaries that are not fully evidenced and can consume identifiers before later persistence fails. | Allocate identifiers and persist the entity/audit atomically in the target. Identifier gaps are allowed; partial entities or falsely reported success are not. |
